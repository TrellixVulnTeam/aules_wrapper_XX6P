...
var CookieJar = /** @class */ (function () {
    function CookieJar() {
        ...
        this.addCookie = function (key, value) {
            var cookie = _this.cookies.find(function (c) { return c.key == key; });
            if (cookie)
                cookie.value = value;
            else
                _this.cookies.push({ key: key, value: value });
        };
        this.removeCookie = function (key) { return _this.cookies.splice(_this.cookies.findIndex(function (c) { return c.key == key; }), 1); };
        this.addCookiesFromHeaders = function (headers) { return headers['set-cookie'].forEach(function (c) {
            var _a = c.split(';')[0].split('='), key = _a[0], value = _a[1];
            if (value == 'deleted')
                _this.removeCookie(key);
            else
                _this.addCookie(key, value);
        }); };
        this.toString = function () { return _this.cookies.map(function (c) { return "".concat(c.key, "=").concat(c.value); }).join('; '); };
    }
    return CookieJar;
}());
...
var Session = /** @class */ (function () {
    function Session(options) {
        ...
        if (options.cookie) {
            options.cookie.split(';').forEach(function (c) {
                var cookie = c.split('=');
                if (cookie[1])
                    _this.cookieJar.addCookie(cookie[0], cookie[1]);
            });
        }
        if (options.course) {
            this.baseURL += "/".concat(options.course);
        }
    }
    Session.prototype.apiRequest = function (method, url, options) {
        ...
        return new Promise(function (resolve, reject) {
            _this.request(__assign({ method: method, url: url, baseURL: _this.baseURL, maxRedirects: 0, headers: __assign({ cookie: _this.cookieJar.toString() }, headers) }, options)).then(function (res) {
                var _a, _b;
                if (typeof (res.data) == 'string')
                    _this.sesskey = (_b = (_a = res.data.match(/sesskey=(\w+)/)) === null || _a === void 0 ? void 0 : _a[1]) !== null && _b !== void 0 ? _b : _this.sesskey;
                res.headers['set-cookie'] && _this.cookieJar.addCookiesFromHeaders(res.headers);
                resolve(res);
                ...
                return __generator(this, function (_b) {
                    switch (_b.label) {
                        case 0:
                            ...
                            if (!(res.status == 303)) return [3 /*break*/, 2];
                            _a = resolve;
                            return [4 /*yield*/, this.apiRequest('GET', res.headers.location)];
                        case 1:
    ...
    Session.prototype.login = function (course, username, password) {
        ...
        return __awaiter(this, void 0, void 0, function () {
            var oldCookieJar, mainPageResponse, loginToken, loginResponse, loginResponseDoc, usermenu, events;
            return __generator(this, function (_d) {
                switch (_d.label) {
                    case 0:
                        this.options.course = course !== null && course !== void 0 ? course : this.options.course;
                        this.options.username = username !== null && username !== void 0 ? username : this.options.username;
                        this.options.password = password !== null && password !== void 0 ? password : this.options.password;
                        oldCookieJar = this.cookieJar;
                        this.cookieJar = new CookieJar();
                        return [4 /*yield*/, this.apiRequest('GET', "/".concat(course, "/login/index.php"))];
                    case 1:
                    ...
                    case 2:
                        ...
                        if (usermenu) {
                            this.baseURL = "https://aules.edu.gva.es/".concat(course);
